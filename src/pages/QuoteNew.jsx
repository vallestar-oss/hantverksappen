  ...
  return (
    <div className="border border-gray-200 rounded-xl p-4 space-y-3 bg-gray-50/50">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className={labelClass}>Antal</label>
          <input
            type="number"
            min="0"
            step="any"
        ...
        <div>
          <label className={labelClass}>À-pris (kr)</label>
          <input
            type="number"
            min="0"
            step="any"
...
export default function QuoteNew() {
  ...
  async function handleSave(status = 'utkast') {
    setError('')
    ...
    // Generate sequential quote number: YYYY-NNN
    ...
    if (rows.length > 0) {
      ...
      if (iErr) {
        console.error('Quote items insert error:', iErr)
  ...
  return (
    <Page className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 px-4 py-4 flex items-center gap-3 sticky top-0 z-10">
        ...
        <h1 className="font-bold text-gray-900 text-lg flex-1 tracking-tight">Ny offert</h1>
        <button
          type="button"
          ...
          <label className="flex items-center gap-3 cursor-pointer select-none">
            <div className="relative">
              <input
                type="checkbox"
                checked={rotRut}
                onChange={e => setRotRut(e.target.checked)}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        name="rotRutType"
                        value={opt.value}
                        checked={rotRutType === opt.value}
                        onChange={() => setRotRutType(opt.value)}
        ...
        <Card title="Sammanställning">
          <div className="space-y-2 text-sm">
            ...
            {rotRut && calc.rotRutDeduction > 0 && (
              <SummaryRow
                label={`${rotRutType.toUpperCase()}-avdrag (30% av arbete)`}
